...
test("createPendingMediaDelete prefers title over file name", () => {
  assert.deepEqual(
    ...
    { count: 1, id: 7, name: "Aurora" }
  );
});

test("getMediaDeleteConfirmMessage falls back to generic text", () => {
  assert.equal(getMediaDeleteConfirmMessage({ count: 1, id: 7, name: "" }), "Are you sure?");
});

test("getMediaDeleteConfirmMessage includes media name when available", () => {
  assert.equal(
    getMediaDeleteConfirmMessage({ count: 1, id: 7, name: "Aurora" }),
    'Are you sure you want to delete "Aurora"?'
  );
});

test("createPendingMediaDelete supports bulk media deletion", () => {
  assert.deepEqual(
    createPendingMediaDelete([{ id: 7 }, { id: 8 }]),
    { count: 2 }
  );
});

test("getMediaDeleteConfirmMessage formats bulk deletion message", () => {
  assert.equal(
    getMediaDeleteConfirmMessage({ count: 3 }),
    "Are you sure you want to delete 3 media items?"
  );
});
